...
class Node(Box):
    def __init__(self, app:App, node):
        ...
        self.node_address._impl.native.set_tooltip_text(address_tooltip)
...
class Peer(Window):
    ...
    def update_peer(self, node):
        ...
        node_box.node_sent._impl.native.set_tooltip_text(last_send)
        node_box.node_receive.text = self.units.format_bytes(node.get('bytesrecv'))
        node_box.node_receive._impl.native.set_tooltip_text(last_receive)
